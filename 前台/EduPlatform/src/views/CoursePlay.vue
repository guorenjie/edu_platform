<!--课程播放 -->
<template>
  <div class="courseplay" style="margin:0px 50px;">
    <div style="width: 70%;margin: 35px auto 50px auto;">
      <video-player
                 v-if="videoSrc"
    ...
    <div class="titleblock" style="height: 53px;">
      <span class="enroltitle">课程答疑</span>
    ...
    <div class="consultlist" v-for="(item,index) in questions" :key="index">
      <div class="consultcontent">
        <div style="height:255px;background: #fff;position: relative;top: 8px;margin:0px 8px;border-radius:20px" @click="jump('/enrolconsultdetail',{wtid:item.wtid,type:'1'})">
          ...
          <img :src="item.twimg | resourcesUrl" style="position: relative;top: 33px;height: 76px;">
...
export default {
  ...
  methods: {
    getCourseInfo() {
      ...
      this.$server.getCourseInfo(paramObj).then((data) => {
        if (data.code === '0') {
          this.courseInfo = JSON.parse(data.courseInfo)
          this.playerOptions.sources[0].src = this.courseInfo[0].kcnr;
          this.videoSrc = this.courseInfo[0].kcnr;
        } else {
    ...
    playerReadied() {
      // 设置初始音量
      document.getElementById('vjs_video_3_html5_api').volume = 0.5
    },
    openAsk() {
      if (this.username === '') {
    ...
    addMyCourse() {
      ...
      this.$server.addMyCourse(paramObj).then((data) => {
        if (data.code === '0') {
          console.info(data.msg);
        } else {
          console.info(data.msg);
